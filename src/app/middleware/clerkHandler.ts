import { currentUser } from '@clerk/nextjs/server'
import { userServices } from '../services/user'

type ClerkHandlerResult = {
  authenticated: boolean
  user: { id: string } | null
}
/** Queries the current clerk context and returns authenticated status and database user info (or adds it if it doesn't exist) */
export default async function clerkHandler(): Promise<ClerkHandlerResult> {
  try {
    const clerkUser = await currentUser()
    if (!clerkUser) {
      return { authenticated: false, user: null }
    }

    const email =
      clerkUser.emailAddresses.length > 0
        ? clerkUser.emailAddresses[0].emailAddress
        : ''
    const username = clerkUser.username ?? ''
    
    const userInDb = await userServices.addOrFetchUserFromClerkId(
      clerkUser.id,
      email,
      username
    )

    if (!userInDb) {
      console.error('Failed to create or fetch user record')
      return { authenticated: false, user: null }
    }

    return { authenticated: true, user: userInDb }
  } catch (error) {
    console.error('ClerkHandler error:', error)
    return { authenticated: false, user: null }
  }
}
