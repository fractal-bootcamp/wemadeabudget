...
export default function AppInfo({ colorMode }: AppInfoProps) {
  ...
  const linkColorClass =
    colorMode === 'light' ? 'text-blue-300' : 'text-blue-600'
  return (
    <div
      className={`flex gap-1 p-2 text-xs ${textColorClass} text-opacity-70 transition-all duration-200`}
    >
      <a
      ...
      >
        <GithubLogo
          className="h-4 w-4"
          color={colorMode === 'light' ? 'white' : '#777777'}
        />
      </a>
      <span>
        Clone of{' '}
