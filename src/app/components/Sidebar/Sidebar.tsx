...
import { useUser, UserButton } from '@/lib/clerkStub'
...
function Sidebar({ setCurrentAccount, setCurrentPage }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div
        className={`flex h-screen transition-all duration-200 ${collapsed ? 'w-[75px]' : 'w-[300px]'} flex-col items-start justify-between bg-[#2c396a] px-2 py-2 font-sans font-light text-white`}
      >
        <div className="flex flex-col gap-2">
          ...
          {!collapsed && (
            <div className="w-full">
              ...
              {showAddAccountModal && (
                <AddAccountModal
                  toggleShowAccountModal={toggleAddAccountModal}
                />
              )}
              {editingAccount && (
                <EditAccountModal
        ...
        <div className="flex w-full flex-row items-center justify-between">
          <div
            className={`h-[48px] w-[70%] overflow-hidden ${showAppInfo ? 'opacity-100' : 'opacity-0'} `}
          >
          ...
          <div
            ...
            onClick={() => {
              ...
              setTimeout(() => setShowAppInfo(collapsed), collapsed ? 200 : 0)
            }}
          >
            {collapsed ? (
