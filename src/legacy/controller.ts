// LEGACY — original Prisma-backed server-action controller. Wraps every
// service in attachUserId, which calls clerkHandler() to derive the userId
// from the Clerk session. Not imported by anything live; preserved so the
// auth/DB pathway can be revived.
